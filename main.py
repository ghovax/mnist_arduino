...
def find_arduino_port():
    ...
    for port in ports:
        ...
        if "Arduino" in port.description or "usbmodem" in port.device:
            logging.info(f"Found Arduino to connect to: {port.description} on {port.device}")
...
def compile_sketch():
    ...
    logging.info(f"Compiling sketch at '{sketch_path}', please wait... it may take up to a few seconds")
...
def flash_arduino(port, hex_file, timeout=30):
    ...
    try:
        if not os.path.exists(hex_file):
            logging.error(f"Cannot find hex file: '{hex_file}', please compile the sketch first")
            return False

        logging.info(f"Flashing Arduino with '{hex_file}'... please wait, the operation will timeout after {timeout}s")
        start_time = time.time()
...
def receive_image_data(arduino, width=175, height=144, timeout=10):
    ...
    logging.info(f"Waiting for image data... please wait, the operation will timeout after {timeout}s")
    ...
    if received_bytes != expected_bytes:
        logging.error(f"Received incomplete data: {received_bytes} vs {expected_bytes} bytes")
        return None
...
def acquire_image(force_compile=False):
    try:
        ...
        if needs_flashing:
            ...
            logging.warning(f"Recompiling the Arduino sketch at '{sketch_path}' ({reason})...")
...
def preprocess_image(image):
    # Convert to grayscale by taking first channel or averaging RGB channels
    if image.shape[-1] == 4:
    ...
    # Reshape to (1, 28, 28) as expected by the model
    image = image.reshape(1, 28, 28)
    
    # Save the preprocessed image for debugging
    debug_image = (image[0] * 255).astype(np.uint8)
    Image.fromarray(debug_image).save('artifacts/debug_mnist_input.png')
...
def main():
    ...
    if acquire_image(force_compile=args.force_compile):
        ...
        threshold_image('artifacts/image.png', 'artifacts/processed_image.png', threshold=100)
